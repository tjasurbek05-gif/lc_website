...
async function main() {
  ...
  console.log(
    `Seeded: ${subjectDefs.length} subjects, ${groupDefs.length} classes, 1 admin, 1 CEO, ${teacherDefs.length} teachers, ${studentDefs.length} students, ${gradeRows.length} grades, ${roomDefs.length} rooms, ${lessonCount} lessons, ${attendanceCount} attendance records, ${payoutCount} teacher payouts, ${paymentCount} tuition payments.`,
  );
